...
- 20k-lemma noun morphology plus 30,809 digest-pinned verb paradigms
  ...
  government;
- 207 seed atoms, 346 semantic relations and 291 curated `FactRecord` values;
- a 141-topic/291-claim audited ResponsePlan V2 corpus with manifest, replay,
...
архитектурный факт дефолтной сборки (сетевой клиент существует только под
фичей `llm-candidates`, unix-socket serve — локальный IPC), а не обещание.

Ежедневная петля:

```bash
# 1. Тема дня: детерминированный выбор из 141 аудированной темы (UTC-день),
...
`reflect` не создаёт базу: при отсутствующем файле карточка строится
без состояния; если база существует, подмешивается карточка памяти
сессии. `report` только читает существующую базу
...
Flip-предложение (`flip-draft` / `flip-verify`, ADR-0043 U6.2) — машинно-
проверяемый пакет, дающий право *предложить* перевод v2-ядра в авторитет:
пять рубрик поверх проверенных FELT-выгрузок (минимум пять различных
устойчивых сессий по десять ходов) и живого перезапуска B2-процедуры
(enabled-рука коммитит, серия нарушений ниже окна гистерезиса, ablated-
контроль давит без нарушений, блокировки охраны совпадают). Черновик
ничего не переключает: сама миграция — ревьюируемое человеком изменение.
...
свободу») выбирается детерминированно из слов самой позиции. Система
отвечает и в самом ходе: запись по теме с сохранённой позицией получает в
ответ цитату этой позиции и возражение графа («Я помню твою позицию…
Граф возражает: …») — выбор тот же детерминированный, поэтому реплей и
верификация экспорта воспроизводят его байт-в-байт. Если новая
...
QxFx0 Rust v0.1.1 health check:
  ...
  [OK] Knowledge pack: active immutable pack fingerprint sha256:…, 291 facts
  [OK] Curated FactRegistry: 291 curated FactId records re-resolve successfully
...
Renderer breadth is measured independently across all 141 audited topics. The
...
It validates all 141 admitted topics in fresh sessions and one shared 141-turn
...
- Recognition covers 141 topics, with declarative rendering admitted for all 141.
- The boundary these replaced is kept as defensive code: should a topic
  ever leave the admitted set, its graph-composed response carries an
  explicit `Граница корпуса` sentence (plus `boundary_marker=true` in the
  render trace) instead of passing as curated knowledge.
- The audited profile contains 291 typed claims across 141 admitted topics; each
...
### Learning bridge and schema v13

Schema v13 adds two beside-the-state tables, `session_bridge_edges` (a
per-session serialized runtime-edge store) and `session_bridge_quarantine`
(per-event refused evidence, append-ordered by `(session_id, seq)`). Both are
owned by the between-turn maintenance path, never the turn path: the bridge's
reinforce/decay/retire ladder and its quarantine ledger write only through
`Persistence::{load,save}_bridge_edges` and `enqueue/load/clear_bridge_
