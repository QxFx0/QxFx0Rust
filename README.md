...
- 141 recognized topics, of which 111 have audited declarative content with
  231 typed claims;
- 20k-lemma noun morphology plus 30,809 digest-pinned verb paradigms
  ...
  government;
- 207 seed atoms, 346 semantic relations and 231 curated `FactRecord` values;
- a 111-topic/231-claim audited ResponsePlan V2 corpus with manifest, replay,
...
```bash
# 1. Тема дня: детерминированный выбор из 111 аудированной темы (UTC-день),
...
QxFx0 Rust v0.1.1 health check:
  ...
  [OK] Content plan assets: recognition_topics_total=141, content_predicates_total=231, argued_topics_admitted=111, argued_predicates_admitted=111, profile_enabled=audited_v1
  ...
  [OK] Knowledge pack: active immutable pack fingerprint sha256:…, 231 facts
  [OK] Curated FactRegistry: 231 curated FactId records re-resolve successfully
...
Renderer breadth is measured independently across all 111 audited topics. The
...
It validates all 111 admitted topics in fresh sessions and one shared 111-turn
...
- Recognition covers 141 topics, with declarative rendering currently admitted for 111.
- The remaining 30 recognized topics are answered from graph composition, and
...
- The audited profile contains 231 typed claims across 111 admitted topics; each
