...
- 141 recognized topics, of which 101 have audited declarative content with
  211 typed claims;
- 20k-lemma noun morphology plus 30,809 digest-pinned verb paradigms
  ...
  government;
- 207 seed atoms, 346 semantic relations and 211 curated `FactRecord` values;
- a 101-topic/211-claim audited ResponsePlan V2 corpus with manifest, replay,
...
```bash
# 1. Тема дня: детерминированный выбор из 101 аудированной темы (UTC-день),
...
QxFx0 Rust v0.1.1 health check:
  ...
  [OK] Content plan assets: recognition_topics_total=141, content_predicates_total=211, argued_topics_admitted=101, argued_predicates_admitted=101, profile_enabled=audited_v1
  ...
  [OK] Knowledge pack: active immutable pack fingerprint sha256:…, 211 facts
  [OK] Curated FactRegistry: 211 curated FactId records re-resolve successfully
...
Renderer breadth is measured independently across all 101 audited topics. The
...
It validates all 101 admitted topics in fresh sessions and one shared 101-turn
...
- Recognition covers 141 topics, with declarative rendering currently admitted for 101.
- The remaining 40 recognized topics are answered from graph composition, and
...
- The audited profile contains 211 typed claims across 101 admitted topics; each
