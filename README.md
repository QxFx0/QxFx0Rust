...
- 141 recognized topics, of which 134 have audited declarative content with
  277 typed claims;
- 20k-lemma noun morphology plus 30,809 digest-pinned verb paradigms
  ...
  government;
- 207 seed atoms, 346 semantic relations and 277 curated `FactRecord` values;
- a 134-topic/277-claim audited ResponsePlan V2 corpus with manifest, replay,
...
```bash
# 1. Тема дня: детерминированный выбор из 134 аудированной темы (UTC-день),
...
QxFx0 Rust v0.1.1 health check:
  ...
  [OK] Content plan assets: recognition_topics_total=141, content_predicates_total=277, argued_topics_admitted=134, argued_predicates_admitted=134, profile_enabled=audited_v1
  ...
  [OK] Knowledge pack: active immutable pack fingerprint sha256:…, 277 facts
  [OK] Curated FactRegistry: 277 curated FactId records re-resolve successfully
...
Renderer breadth is measured independently across all 134 audited topics. The
...
It validates all 134 admitted topics in fresh sessions and one shared 134-turn
...
- Recognition covers 141 topics, with declarative rendering currently admitted for 134.
- The remaining 7 recognized topics are answered from graph composition, and
...
- The audited profile contains 277 typed claims across 134 admitted topics; each
